...
for(root,dirs,files)in os.walk(dir_name):
	...
	json_files=[]
	for dir_name in scan_dirs:json_files.extend(glob.glob(os.path.join(dir_name,'**','Streaming_History_Audio_*.json'),recursive=True));json_files=[os.path.relpath(file,current_dir)for file in json_files];selected_files=json_files
os.system('cls'if os.name=='nt'else'clear')
data=[]
missing_ms_played_files=[]
downloads_dir=os.path.join(os.path.expanduser('~'),'Downloads')
for file in selected_files:file_name=os.path.basename(file);file_dir=os.path.join(downloads_dir,'Spotify Extended Streaming History',file_name);df=pd.read_json(file_dir);data.append(df);print(f"Read data from file: {file_name}")
os.system('cls'if os.name=='nt'else'clear')
df=pd.concat(data,ignore_index=True)
if'ms_played'in df.columns:df['ms_played']=df['ms_played']/60000
else:print(f"ms_played column not found in the following JSON files: {', '.join(missing_ms_played_files)}")
...
for artist in top_artists.index:artist_data=df[df['master_metadata_album_artist_name']==artist];top_songs=artist_data.groupby('master_metadata_track_name')['ms_played'].sum().sort_values(ascending=False).head(10);total_streaming_time=artist_data['ms_played'].sum();first_time_streamed=artist_data['ts'].min();different_tracks=artist_data['master_metadata_track_name'].nunique();num_artists=10;num_songs=10;top_artists=top_50_artists;top_tracks=top_50_tracks
